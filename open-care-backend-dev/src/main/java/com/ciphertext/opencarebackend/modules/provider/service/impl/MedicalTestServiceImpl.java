...
import com.ciphertext.opencarebackend.modules.provider.repository.MedicalTestRepository;
...
import static com.ciphertext.opencarebackend.modules.shared.repository.specification.QueryOperator.EQUALS;
import static com.ciphertext.opencarebackend.modules.shared.repository.specification.QueryOperator.LIKE;
...
public class MedicalTestServiceImpl implements MedicalTestService {
    ...
    public MedicalTest createMedicalTest(MedicalTest medicalTest) {
        ...
        if (medicalTest.getHospitalCount() == null) {
            medicalTest.setHospitalCount(0);
        }
    ...
    public void refreshAll() {
        log.info("Refreshing hospital counts for all medical tests");
        medicalTestRepository.refreshHospitalCounts();
        log.info("Successfully refreshed hospital counts for all medical tests");
    }

    @Override
    @Transactional
    public void refreshOne(int id) {
        log.info("Refreshing hospital count for medical test with id: {}", id);
        medicalTestRepository.refreshHospitalCountById(id);
        log.info("Successfully refreshed hospital count for medical test with id: {}", id);
    }
}
